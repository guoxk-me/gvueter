...
function addInternalRouteIssues(
  ...
  if (path && !getNavigationRoutePath(path, ''))
    fieldErrors.path = 'INVALID_PATH'
  if (routeName && !isNavigationRouteName(routeName))
    fieldErrors.routeName = 'INVALID_ROUTE_NAME'
...
export function getManagedMenuTargetDecision(
  ...
  if (fields.kind === 'menu') {
    if (targetUrl)
      fieldErrors.targetUrl = 'TARGET_URL_NOT_ALLOWED_FOR_MENU'
    ...
    if (routeFieldCount > 0 && routeFieldCount < 3) {
      if (!path)
        fieldErrors.path = 'PATH_REQUIRED'
      if (!routeName)
        fieldErrors.routeName = 'ROUTE_NAME_REQUIRED'
      if (!componentKey)
        fieldErrors.componentKey = 'COMPONENT_REQUIRED'
    }
    if (routeFieldCount === 3)
    ...
    if (!urlDecision.isAllowed)
      fieldErrors.targetUrl = urlDecision.reason

    if (fields.kind === 'external') {
      if (path)
        fieldErrors.path = 'ROUTE_FIELD_NOT_ALLOWED'
      if (routeName)
        fieldErrors.routeName = 'ROUTE_FIELD_NOT_ALLOWED'
      if (componentKey)
        fieldErrors.componentKey = 'ROUTE_FIELD_NOT_ALLOWED'
    }
    else {
      if (!path)
        fieldErrors.path = 'PATH_REQUIRED'
      ...
      if (componentKey !== 'iframe')
        fieldErrors.componentKey = 'IFRAME_COMPONENT_REQUIRED'
      addInternalRouteIssues(fieldErrors, path, routeName, componentKey)
    }
  }
