...
function isBackendMenuResponse(value: unknown): value is BackendMenuResponse {
  ...
  while (pendingNodes.length > 0) {
    ...
    if (!current || current.depth > MAX_NAVIGATION_DEPTH || !isRecord(current.node)) return false
    if (visitedNodes.has(current.node)) return false
    visitedNodes.add(current.node)
    nodeCount += 1
    if (nodeCount > MAX_NAVIGATION_NODES) return false
