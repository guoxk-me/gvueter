...
if (isMockBuild) {
  if (!browserMockEntry) violations.push('Mock build is missing the browser handler entry.')
  if (!hasMockWorker) violations.push('Mock build is missing mockServiceWorker.js.')
} else {
  if (browserMockEntry) violations.push('Production build contains the browser Mock handler entry.')
  if (hasMockWorker) violations.push('Production build contains mockServiceWorker.js.')
}
...
// AI modified: keep CI output compact while proving that every emitted JavaScript entry is budgeted.
process.stdout.write(`Bundle budget passed for ${javascriptAssets.length} JavaScript assets.\n`)
