...
<!-- AI modified: skeleton decisions remain in the retained architecture and deployment documents. -->
...
- When a decision changed, update the relevant product documents in the same turn before the final response. At minimum, check `README.md`, `docs/admin-architecture.md`, `docs/deployment.md`, and the applicable engineering document.
...
`pnpm run verify` covers toolchain diagnostics, TypeScript, ESLint, documentation, runtime configuration, security, coverage, the production audit, and a production build. `pnpm run release:check` adds three-browser E2E and Base-scoped PWA verification, then restores a standard production `dist` even after failure. See [Testing](./docs/testing.md) for the evidence policy.
